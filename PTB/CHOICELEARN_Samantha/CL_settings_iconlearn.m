...
% Define the radius of the circle (in degrees visual angle)
circle_radius = 5;

% Number of stimuli
num_stimuli = 6;

% Calculate the positions for each stimulus
angles = linspace(0, 2*pi, num_stimuli + 1);
angles(end) = []; % Remove the last angle to avoid overlap

positions = [circle_radius * cos(angles)' circle_radius * sin(angles)'];

% TRIALS ----------------
% Update to 6 images per trial
STIM.Trials.trial(1).images = [1, 5, 13, 9, 2, 10]; 
% Assign calculated positions
STIM.Trials.trial(1).imgpos = positions;
...
STIM.Trials.trial(2).images = [10, 14, 6, 2, 11, 15]; 
STIM.Trials.trial(2).imgpos = positions; % Reuse the same positions
