...
% for trial_num = 1:length(LOG.TrialList)  % Adjust as needed for the number of trials
%     % Use the current relevant and redundant images
%     relevant_index = current_relevant_index;
...
% 
%     % Combine relevant, redundant, and distractors
%     % Ensure correct indices for distractor images
%     distractor_images = distractor_indices + length(relevant_files) + length(redundant_files);
%     images = [relevant_index, redundant_index, distractor_images];
% 
%     % Randomize the positions
%     randomized_positions = positions(randperm(num_images), :);
% 
...
% trials_in_block = 0; % Initialize the trial counter
% 
% % % Load the first images for the initial blocks (with this piece of code,
% % it does not take the first image of the block)
% % for i = 1:5
